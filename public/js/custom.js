(function($) { 
	...
	var W3cms = function(){
		...
		var handleReadingConfigs = function() {
			var readingForm = $('#export-filters'),
			filters = readingForm.find('.export-filters');
			filters.hide();
			readingForm.find('input:radio').on( 'change', function() {
				filters.slideUp('fast');
				switch ( $(this).val() ) {
					case 'Page': $('#page-filters').slideDown(); break;
				}
			});
			if($('#show_on_front_page').is(':checked')) { $('#page-filters').slideDown(); }
		}
