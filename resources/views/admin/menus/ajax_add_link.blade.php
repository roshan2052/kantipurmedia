		...
		<div class="accordion__item">
			...
			<div id="rounded-{{ $menuItem->id }}" class="collapse accordion__body p-3" data-bs-parent="#accordion-menu-structure">
				<div class="row">
					...
					<div class="col-md-6 form-group">
						<label for="MenuItem{{ $menuItem->id }}Attribute">{{ __('Title Attribute') }}</label>
					...
					<div class="col-md-12 form-group XLinkTarget" {{ $screenOption['LinkTarget']['visibility'] ? '' : 'd-none' }}>
						<div class="form-check">
							...
							<label class="form-check-label" for="MenuItem{{ $menuItem->id }}Target">{{ __('Open link in a new tab') }}</label>
						</div>
					</div>
					<div class="col-md-12 form-group XCssClasses" {{ $screenOption['CssClasses']['visibility'] ? '' : 'd-none' }}>
						<label for="MenuItem[{{ $menuItem->id }}]CssClasses">{{ __('CSS Classes') }}</label>
					...
					<div class="col-md-12 form-group XDescription" {{ $screenOption['Description']['visibility'] ? '' : 'd-none' }}>
						<label for="MenuItem[{{ $menuItem->id }}]Description">{{ __('Description') }}</label>
					...
					@endif
					<div class="col-md-12">
						<a href="javascript:void(0);" class="RemoveItem text-primary" rel="{{ $menuItem->id }}" item-name="{{ ucfirst($menuItem->title) }}">{{ __('Remove') }}</a>
						<span class="text-black mx-2">|</span>
						<a href="javascript:void(0);" rel="{{ $menuItem->id }}" class="CancelItem text-primary">{{ __(' Cancel') }}</a>
					</div>
				</div>
			</div>
		</div>
